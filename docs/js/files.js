...
document.addEventListener("DOMContentLoaded", function () {
	...
	let asideMenuArrows = document.querySelectorAll('.aside-menu__arrow');
	if (asideMenuArrows) {
		for (const arrow of asideMenuArrows) {
			arrow.addEventListener('click', function () {
				if (this.classList.contains('_active')) {
					_slideUp(this.parentElement.nextElementSibling, 400, this);
					this.classList.remove('_active');
					this.parentElement.classList.remove('_active');
				} else {
					_slideDown(this.parentElement.nextElementSibling, 400, this);
...
function clickHendler(e) {
	...
	if (target.closest('.search-form__button_remove')) {
		let searchInput = document.querySelector('.search-form__input');
		searchInput.value = searchInput.dataset.value;
		document.querySelector('.search-form__input').blur();
	}

	if (target.closest('.watch')) {
		target.closest('.watch').classList.toggle('_active');
	}
}
